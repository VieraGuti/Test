(() => {
  ...
  const partPaths=Array.from({length:9},(_,i)=>`./v04/part-${String(i+1).padStart(2,'0')}.txt?v=515`);
  const patchPaths=Array.from({length:21},(_,i)=>`./v05/patch-${String(i+1).padStart(2,'0')}.txt?v=515`);
  ...
  Promise.all([Promise.all(partPaths.map(getText)),Promise.all(patchPaths.map(getText))])
    .then(([parts,patchParts])=>{
      const patch=patchParts.join('');let source=parts.join('');
      source=replaceRequired(source,"function updatePlayer(dt){const joy=touchMove;","function updatePlayer(dt){if(isTouch)runHeld=Math.hypot(touchMove.x,touchMove.y)>.82;const joy=touchMove;",'sprint analógico');
      source=replaceRequired(source,"const sprint=(runHeld||keys.has('ShiftLeft'))&&f>.2&&player.stamina>1,speed=sprint?7.1:4.55;if(sprint)player.stamina=clamp(player.stamina-dt*18,0,100);else player.stamina=clamp(player.stamina+dt*11,0,100);","const sprint=(runHeld||keys.has('ShiftLeft'))&&player.stamina>1,speed=sprint?7.1:4.55;if(sprint)player.stamina=clamp(player.stamina-dt*7.5,0,100);else player.stamina=clamp(player.stamina+dt*14,0,100);",'stamina prolongada');
      ...
      source=replaceRequired(source,"toast(save?'VAST v0.4 · Partida restaurada':'VAST v0.4 · EQUÍPATE, FARMEA, CONSTRUYE');","toast(save?'VAST v0.5.15 · Partida restaurada':'VAST v0.5.15 · ADS LIMPIO + VIEWMODEL PRO');",'mensaje de versión');
    ...
    .catch(error=>{console.error('[VAST v0.5.15] Error de arranque',error);document.getElementById('loading')?.classList.add('hidden');const box=document.createElement('div');box.style.cssText='position:fixed;z-index:9999;inset:18%;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px;background:#0b0d0c;border:1px solid #713f2b;color:#eee;font:600 14px system-ui';box.textContent='VAST v0.5.15 no pudo cargar. Recarga la página para reintentar.';document.body.appendChild(box)});
})();
