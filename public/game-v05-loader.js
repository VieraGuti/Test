(() => {
  ...
  Promise.all([Promise.all(partPaths.map(getText)),Promise.all(patchPaths.map(getText))])
    .then(([parts,patchParts])=>{
      ...
      source=replaceRequired(source,"toast(save?'VAST v0.4 · Partida restaurada':'VAST v0.4 · EQUÍPATE, FARMEA, CONSTRUYE');","toast(save?'VAST v0.5.14 · Partida restaurada':'VAST v0.5.14 · SCALE + GROUND SNAP + FOUNDATIONS');",'mensaje de versión');
    ...
    .catch(error=>{console.error('[VAST v0.5.14] Error de arranque',error);document.getElementById('loading')?.classList.add('hidden');const box=document.createElement('div');box.style.cssText='position:fixed;z-index:9999;inset:18%;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px;background:#0b0d0c;border:1px solid #713f2b;color:#eee;font:600 14px system-ui';box.textContent='VAST v0.5.14 no pudo cargar. Recarga la página para reintentar.';document.body.appendChild(box)});
})();
