(() => {
  ...
  const partPaths = Array.from({ length: 9 }, (_, i) => `./v04/part-${String(i + 1).padStart(2, '0')}.txt?v=504`);
  const patchPaths = ['./v05/patch-01.txt?v=504','./v05/patch-02.txt?v=504','./v05/patch-03.txt?v=504','./v05/patch-04.txt?v=504','./v05/patch-05.txt?v=504','./v05/patch-06.txt?v=504'];
  ...
  Promise.all([Promise.all(partPaths.map(getText)), Promise.all(patchPaths.map(getText))])
    .then(([parts, patchParts]) => {
      ...
      source = replaceRequired(source,
        ...
        "toast(save?'VAST v0.5 · Partida restaurada':'VAST v0.5 · BUILD PRO + HELI PILOTABLE');",
        'mensaje de versión');
    ...
    .catch((error) => {
      console.error('[VAST v0.5] Error de arranque', error);
      ...
      box.textContent = 'VAST v0.5 no pudo cargar. Recarga la página para reintentar.';
