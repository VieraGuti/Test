(() => {
  ...
  const partPaths = Array.from({ length: 9 }, (_, i) => `./v04/part-${String(i + 1).padStart(2, '0')}.txt?v=506`);
  const patchPaths = ['./v05/patch-01.txt?v=506','./v05/patch-02.txt?v=506','./v05/patch-03.txt?v=506','./v05/patch-04.txt?v=506','./v05/patch-05.txt?v=506','./v05/patch-06.txt?v=506','./v05/patch-07.txt?v=506','./v05/patch-08.txt?v=506','./v05/patch-09.txt?v=506'];
  ...
  Promise.all([Promise.all(partPaths.map(getText)), Promise.all(patchPaths.map(getText))])
    .then(([parts, patchParts]) => {
      ...
      source = replaceRequired(source,
        ...
        "toast(save?'VAST v0.5.5 · Partida restaurada':'VAST v0.5.5 · RESPAWN + FOGATA SEGURA + HELI V2');",
        'mensaje de versión');
    ...
    .catch((error) => {
      console.error('[VAST v0.5.5] Error de arranque', error);
      ...
      box.textContent = 'VAST v0.5.5 no pudo cargar. Recarga la página para reintentar.';
